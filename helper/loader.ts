...
import fs from 'fs'
import { Time, getFileLinesSync, constants } from '@mt-inc/utils'
import Zip from 'adm-zip'
import Binance from 'binance-api-node'
import sha256File from 'sha256-file'
import Positions, { Candle } from '@mt-inc/bot'
import { TrixBot } from '@mt-inc/strategy'
...
export class Loader {
  ...
  async loadData() {
    try {
      ...
      if (
        startDate.getUTCDate() === todayDate.getUTCDate() &&
        startDate.getUTCMonth() === todayDate.getUTCMonth() &&
        startDate.getUTCFullYear() === todayDate.getUTCFullYear()
      ) {
        api = { ...api, active: true }
      } else {
        const timeEndDate = new Date(end)
        timeEndDate.setUTCHours(23)
        timeEndDate.setUTCMinutes(59)
        timeEndDate.setUTCSeconds(59)
        timeEndDate.setUTCMilliseconds(999)
        let timeEnd = timeEndDate.getTime()
        if (
          endDate.getUTCDate() === todayDate.getUTCDate() &&
          endDate.getUTCMonth() === todayDate.getUTCMonth() &&
          endDate.getUTCFullYear() === todayDate.getUTCFullYear()
        ) {
          const endTime = new Date(end)
          endTime.setUTCDate(endDate.getUTCDate() - 1)
          ...
          api = { ...api, active: true, start: startDate.getTime() }
        }
        for (let i = start; i <= timeEnd; i = i + 24 * 60 * 60 * 1000) {
          const d = new Date(i)
          const y = d.getUTCFullYear()
          const m = d.getUTCMonth() + 1
          const D = d.getUTCDate()
          fileNames.push(
            `${pair}-aggTrades-${y}-${m < 10 ? `0${m}` : m}-${
              D < 10 ? `0${D}` : D
            }.zip`
          )
        }
      ...
      if (fileNames.length > 0) {
        ...
        if (fileNames.length > existFiles.length && existFiles.length > 0) {
          ...
          apiStartDate.setUTCMonth(
            parseInt(splitDateArr[splitDateArr.length - 2]) - 1
          )
          apiStartDate.setUTCFullYear(
            parseInt(splitDateArr[splitDateArr.length - 3])
          )
          ...
          api = {
            ...
            active: true,
            start:
              apiStartDate.getTime() < start ? start : apiStartDate.getTime(),
          }
        }
        fileNames.length = 0
        fileNames.push(...existFiles)
        if (fileNames.length > 0) {
          this.sendMessage({
            event: 'loaderEvent',
            text: `Підготовку завершено.${
              fileNames.length > 0
                ? `</br>Буде завантажено файлів: ${fileNames.length}.`
                : ''
            }${
              api.active
                ? `</br>Дані з ${this.time.format(
                    api.start
                  )} по ${this.time.format(api.end)} буде завантажено по API.`
                : ''
            }`,
            progress: 0.00001,
            step: 1,
          })
          await this.loadFiles(fileNames)
          this.sendMessage({
            event: 'loaderEvent',
          ...
          await this.unzipFiles(fileNames)
        }
      }
      if (api.active) {
        this.sendMessage({
      ...
      this.sendMessage({
        event: 'loaderEvent',
        text: `Початок симуляції</br>В період з ${this.time.format(
          resNow?.start || start
        )} до ${this.time.format(
          resNow?.end || end
  ...
  private getFiles() {
    ...
    return fs
      ...
      .filter((item) => {
        ...
        return (
          splitDate.getTime() > startDate.getTime() &&
          splitDate.getTime() < endDate.getTime()
        )
      })
  }
  async checkFiles(fileNames: string[]) {
    try {
      const checkFiles = fileNames.map(async (file) => {
        if (!this.cancel) {
          const req = await fetch(`${this.url}/${file}.CHECKSUM`).then(
            (res: any) => res
          )
          if (req.status !== 200) {
    ...
    const downloadFiles = fileNames.map(async (file) => {
      if (!this.cancel) {
        ...
        if (fs.existsSync(save)) {
          ...
          const savedCheckSum = this.checksums.filter(
            (item) => item.split('  ')[1] === file
          )[0]
        ...
        if (load) {
          const req = await fetch(`${this.url}/${file}`).then((res: any) =>
            res.blob()
          )
  ...
  async loadAPIData(
    ...
    if (startTime < endTime && !this.cancel) {
      try {
        ...
        const toWrite = res.filter(
          (item) => item.timestamp < lastTime && item.timestamp < endTime
        )
        c += toWrite.length
        ...
        await new Promise((res) =>
          res(this.loadAPIData({ startTime: lastTime, endTime }, c, fR))
        )
      } catch (e: any) {
        if (e.code === 'ENOTFOUND' && e.erroredSysCall === 'getaddrinfo') {
          throw e
        ...
        await new Promise((res) =>
          setTimeout(
            () => res(this.loadAPIData({ startTime, endTime }, c, fR)),
            5000
          )
        )
      }
    ...
    if (!this.cancel) {
      ...
      for (let i = 0; i < files.length; i++) {
        if (!this.cancel) {
          ...
          for (const d of data) {
            if (!this.cancel) {
              let [_aggId, p, v, _firstId, _lastId, time, _wasMaker] =
                d.split(',')
